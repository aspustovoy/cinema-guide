import { FC, useEffect, useState } from 'react';
import './index.css';

const Footer: FC = () => {
  const [width, setWidth] = useState(window.innerWidth);
  const updateDimensions = () => {
    setWidth(window.innerWidth);
  };

  useEffect(() => {
    window.addEventListener('resize', updateDimensions);
    return () => window.removeEventListener('resize', updateDimensions);
  }, []);

  return (
    <footer className="footer">
      <div className="container footer__container">
        <div className="footer__text">
          <span className="footer__str1">LLC «Мультимедиа Визион»</span>
          <span className="footer__str2">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
                d="M16.2877 9.42773C15.413 7.97351 13.8195 7 12 7C9.23999 7 7 9.23999 7 12C7 14.76 9.23999 17 12 17C13.8195 17 15.413 16.0265 16.2877 14.5723L14.5729 13.5442C14.0483 14.4166 13.0927 15 12 15C10.3425 15 9 13.6575 9 12C9 10.3425 10.3425 9 12 9C13.093 9 14.0491 9.58386 14.5735 10.4568L16.2877 9.42773ZM22 12C22 6.47998 17.52 2 12 2C6.47998 2 2 6.47998 2 12C2 17.52 6.47998 22 12 22C17.52 22 22 17.52 22 12ZM4 12C4 7.57996 7.57996 4 12 4C16.42 4 20 7.57996 20 12C20 16.42 16.42 20 12 20C7.57996 20 4 16.42 4 12Z"
                fill="white"
                fillOpacity="0.7"
              />
            </svg>{' '}
            Все права защищены
          </span>
        </div>
        <ul className="list-reset footer__list">
          <li className="footer__item">
            <a href="https://vk.com/" className="footer__link">
              {width <= 768 ? (
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <rect x="0.5" y="0.5" width="23" height="23" rx="7.5" stroke="white" strokeOpacity="0.8" />
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M17.659 9.58645C17.7515 9.3234 17.659 9.13208 17.2492 9.13208H15.8878C15.5442 9.13208 15.3856 9.29947 15.2931 9.47886C15.2931 9.47886 14.5926 11.0094 13.6145 12.0018C13.2973 12.2888 13.1519 12.3845 12.9801 12.3845C12.8875 12.3845 12.7686 12.2888 12.7686 12.0257V9.57449C12.7686 9.26361 12.6629 9.12012 12.3721 9.12012H10.2309C10.0194 9.12012 9.88724 9.26361 9.88724 9.4071C9.88724 9.70604 10.3763 9.77777 10.4291 10.6148V12.4323C10.4291 12.8269 10.3498 12.8986 10.178 12.8986C9.7154 12.8986 8.59194 11.3561 7.91786 9.59842C7.78567 9.25164 7.65351 9.12012 7.30988 9.12012H5.93526C5.53875 9.12012 5.47266 9.28754 5.47266 9.46689C5.47266 9.78974 5.93526 11.4159 7.62706 13.5682C8.75055 15.039 10.3498 15.8282 11.7905 15.8282C12.6629 15.8282 12.7686 15.6488 12.7686 15.3499V14.2379C12.7686 13.8791 12.8479 13.8194 13.1255 13.8194C13.3237 13.8194 13.6806 13.915 14.4868 14.6205C15.412 15.4575 15.5706 15.8401 16.0861 15.8401H17.4475C17.844 15.8401 18.0291 15.6608 17.9233 15.314C17.8044 14.9672 17.355 14.465 16.7734 13.8672C16.4562 13.5324 15.9804 13.1617 15.835 12.9823C15.6367 12.7432 15.6896 12.6475 15.835 12.4323C15.8218 12.4323 17.4871 10.3039 17.659 9.58645Z"
                    fill="white"
                    fillOpacity="0.8"
                  />
                </svg>
              ) : (
                <svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <rect x="0.5" y="0.5" width="35" height="35" rx="7.5" stroke="white" strokeOpacity="0.8" />
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M26.4865 14.3797C26.6253 13.9851 26.4865 13.6981 25.8719 13.6981H23.8298C23.3144 13.6981 23.0764 13.9492 22.9377 14.2183C22.9377 14.2183 21.8869 16.5141 20.4198 18.0027C19.9439 18.4332 19.7259 18.5767 19.4682 18.5767C19.3293 18.5767 19.1509 18.4332 19.1509 18.0386V14.3617C19.1509 13.8954 18.9923 13.6802 18.5561 13.6802H15.3444C15.0271 13.6802 14.8289 13.8954 14.8289 14.1106C14.8289 14.5591 15.5624 14.6667 15.6417 15.9222V18.6484C15.6417 19.2403 15.5228 19.3479 15.2651 19.3479C14.5711 19.3479 12.886 17.0342 11.8748 14.3976C11.6765 13.8775 11.4783 13.6802 10.9629 13.6802H8.90093C8.30617 13.6802 8.20703 13.9313 8.20703 14.2003C8.20703 14.6846 8.90093 17.1239 11.4386 20.3524C13.1239 22.5585 15.5228 23.7422 17.6838 23.7422C18.9923 23.7422 19.1509 23.4732 19.1509 23.0248V21.3568C19.1509 20.8187 19.2699 20.729 19.6862 20.729C19.9836 20.729 20.5189 20.8725 21.7283 21.9307C23.1161 23.1862 23.354 23.7602 24.1272 23.7602H26.1693C26.764 23.7602 27.0416 23.4911 26.883 22.971C26.7046 22.4509 26.0305 21.6976 25.1582 20.8007C24.6823 20.2986 23.9686 19.7426 23.7505 19.4735C23.4531 19.1148 23.5324 18.9713 23.7505 18.6484C23.7307 18.6484 26.2287 15.4559 26.4865 14.3797Z"
                    fill="white"
                    fillOpacity="0.8"
                  />
                </svg>
              )}
            </a>
          </li>

          <li className="footer__item">
            <a href="https://ok.ru/" className="footer__link">
              {width <= 768 ? (
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <rect x="0.5" y="0.5" width="23" height="23" rx="7.5" stroke="white" strokeOpacity="0.8" />
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M16.0015 8.69971C16.4421 8.82062 16.7891 9.17687 16.9069 9.62927C17.1209 10.4492 17.1209 12.16 17.1209 12.16C17.1209 12.16 17.1209 13.8707 16.9069 14.6907C16.7891 15.1431 16.4421 15.4993 16.0015 15.6203C15.203 15.84 12.0009 15.84 12.0009 15.84C12.0009 15.84 8.79871 15.84 8.00016 15.6203C7.55955 15.4993 7.21255 15.1431 7.09479 14.6907C6.88086 13.8707 6.88086 12.16 6.88086 12.16C6.88086 12.16 6.88086 10.4492 7.09479 9.62927C7.21255 9.17687 7.55955 8.82062 8.00016 8.69971C8.79871 8.47998 12.0009 8.47998 12.0009 8.47998C12.0009 8.47998 15.203 8.47998 16.0015 8.69971ZM11.0408 10.7201V13.9201L13.6008 12.3202L11.0408 10.7201Z"
                    fill="white"
                    fillOpacity="0.8"
                  />
                </svg>
              ) : (
                <svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <rect x="0.5" y="0.5" width="35" height="35" rx="7.5" stroke="white" strokeOpacity="0.8" />
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M24.0013 13.0496C24.6622 13.2309 25.1827 13.7653 25.3593 14.4439C25.6803 15.6738 25.6803 18.24 25.6803 18.24C25.6803 18.24 25.6803 20.8061 25.3593 22.036C25.1827 22.7146 24.6622 23.249 24.0013 23.4305C22.8035 23.76 18.0003 23.76 18.0003 23.76C18.0003 23.76 13.1971 23.76 11.9993 23.4305C11.3383 23.249 10.8179 22.7146 10.6412 22.036C10.3203 20.8061 10.3203 18.24 10.3203 18.24C10.3203 18.24 10.3203 15.6738 10.6412 14.4439C10.8179 13.7653 11.3383 13.2309 11.9993 13.0496C13.1971 12.72 18.0003 12.72 18.0003 12.72C18.0003 12.72 22.8035 12.72 24.0013 13.0496ZM16.5603 16.0802V20.8802L20.4003 18.4803L16.5603 16.0802Z"
                    fill="white"
                    fillOpacity="0.8"
                  />
                </svg>
              )}
            </a>
          </li>

          <li className="footer__item">
            <a href="https://www.youtube.com/" className="footer__link">
              {width <= 768 ? (
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <rect x="0.5" y="0.5" width="23" height="23" rx="7.5" stroke="white" strokeOpacity="0.8" />
                  <path
                    d="M13.2787 14.8416L15.0477 16.5493C15.41 16.8984 15.41 17.4653 15.0477 17.8148C14.6856 18.1642 14.0989 18.1642 13.7373 17.8148L11.998 16.1366L10.2603 17.8148C10.0791 17.9893 9.84168 18.0767 9.60425 18.0767C9.3672 18.0767 9.13015 17.9893 8.94896 17.8148C8.58695 17.4653 8.58695 16.8987 8.94858 16.5493L10.7177 14.8416C10.0736 14.6999 9.4524 14.4535 8.88023 14.107C8.44722 13.8435 8.31714 13.2914 8.58998 12.873C8.86205 12.4541 9.43423 12.328 9.86799 12.5916C11.1634 13.3782 12.8322 13.3783 14.1284 12.5916C14.5622 12.328 15.1342 12.4541 15.4068 12.873C15.6797 13.291 15.5492 13.8435 15.1162 14.107C14.544 14.4539 13.9228 14.6999 13.2787 14.8416Z"
                    fill="white"
                    fillOpacity="0.8"
                  />
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M8.82617 9.2947C8.82617 10.9787 10.2454 12.3485 11.9905 12.3485C13.736 12.3485 15.1549 10.9787 15.1549 9.2947C15.1549 7.61017 13.736 6.23999 11.9905 6.23999C10.2454 6.23999 8.82617 7.61017 8.82617 9.2947ZM13.3007 9.29438C13.3007 8.59695 12.713 8.02984 11.9905 8.02984C11.2686 8.02984 10.6803 8.59695 10.6803 9.29438C10.6803 9.99126 11.2686 10.5587 11.9905 10.5587C12.713 10.5587 13.3007 9.99126 13.3007 9.29438Z"
                    fill="white"
                    fillOpacity="0.8"
                  />
                </svg>
              ) : (
                <svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <rect x="0.5" y="0.5" width="35" height="35" rx="7.5" stroke="white" strokeOpacity="0.8" />
                  <path
                    d="M19.919 22.2622L22.5725 24.8238C23.116 25.3474 23.116 26.1978 22.5725 26.722C22.0294 27.2462 21.1493 27.2462 20.6069 26.722L17.998 24.2048L15.3914 26.722C15.1196 26.9838 14.7635 27.1149 14.4074 27.1149C14.0518 27.1149 13.6962 26.9838 13.4244 26.722C12.8814 26.1978 12.8814 25.348 13.4238 24.8238L16.0776 22.2622C15.1114 22.0497 14.1796 21.6802 13.3213 21.1604C12.6718 20.7651 12.4767 19.9369 12.8859 19.3094C13.2941 18.681 14.1523 18.4919 14.803 18.8872C16.7461 20.0671 19.2493 20.0674 21.1936 18.8872C21.8443 18.4919 22.7023 18.681 23.1112 19.3094C23.5205 19.9364 23.3248 20.7651 22.6753 21.1604C21.817 21.6807 20.8852 22.0497 19.919 22.2622Z"
                    fill="white"
                    fillOpacity="0.8"
                  />
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M13.2402 13.9419C13.2402 16.4679 15.3691 18.5226 17.9868 18.5226C20.605 18.5226 22.7333 16.4679 22.7333 13.9419C22.7333 11.4151 20.605 9.35986 17.9868 9.35986C15.3691 9.35986 13.2402 11.4151 13.2402 13.9419ZM19.952 13.9414C19.952 12.8953 19.0705 12.0446 17.9867 12.0446C16.9038 12.0446 16.0214 12.8953 16.0214 13.9414C16.0214 14.9868 16.9038 15.838 17.9867 15.838C19.0705 15.838 19.952 14.9868 19.952 13.9414Z"
                    fill="white"
                    fillOpacity="0.8"
                  />
                </svg>
              )}
            </a>
          </li>

          <li className="footer__item">
            <a href="https://web.telegram.org/" className="footer__link">
              {width <= 768 ? (
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <rect x="0.5" y="0.5" width="23" height="23" rx="7.5" stroke="white" strokeOpacity="0.8" />
                  <path
                    d="M5.73404 11.9059C6.85627 11.2878 8.10899 10.7718 9.27947 10.2533C11.2932 9.40393 13.3148 8.56929 15.3569 7.79225C15.7542 7.65985 16.4681 7.53037 16.5381 8.11917C16.4998 8.95261 16.3421 9.78117 16.234 10.6097C15.9594 12.4318 15.6422 14.2476 15.3328 16.0637C15.2262 16.6686 14.4684 16.9818 13.9835 16.5947C12.8182 15.8076 11.644 15.0281 10.4936 14.2228C10.1167 13.8398 10.4662 13.29 10.8027 13.0165C11.7625 12.0707 12.7803 11.2671 13.6899 10.2724C13.9353 9.67985 13.2103 10.1792 12.9712 10.3322C11.6573 11.2376 10.3755 12.1984 8.99023 12.9941C8.28263 13.3836 7.45791 13.0508 6.75063 12.8334C6.11646 12.5708 5.18718 12.3063 5.73398 11.906L5.73404 11.9059Z"
                    fill="white"
                    fillOpacity="0.8"
                  />
                </svg>
              ) : (
                <svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <rect x="0.5" y="0.5" width="35" height="35" rx="7.5" stroke="white" strokeOpacity="0.8" />
                  <path
                    d="M8.60008 17.8588C10.2834 16.9315 12.1625 16.1576 13.9182 15.3798C16.9388 14.1058 19.9713 12.8538 23.0344 11.6882C23.6304 11.4896 24.7012 11.2954 24.8062 12.1786C24.7487 13.4288 24.5122 14.6716 24.35 15.9145C23.9382 18.6476 23.4623 21.3713 22.9982 24.0955C22.8383 25.0028 21.7016 25.4726 20.9742 24.8919C19.2263 23.7112 17.465 22.5421 15.7394 21.334C15.1741 20.7596 15.6983 19.9348 16.2031 19.5247C17.6427 18.1059 19.1695 16.9005 20.5339 15.4084C20.9019 14.5196 19.8145 15.2687 19.4558 15.4982C17.4849 16.8563 15.5623 18.2974 13.4844 19.4911C12.423 20.0753 11.1859 19.576 10.125 19.25C9.17372 18.8561 7.77979 18.4594 8.59999 17.8588L8.60008 17.8588Z"
                    fill="white"
                    fillOpacity="0.8"
                  />
                </svg>
              )}
            </a>
          </li>
        </ul>
      </div>
    </footer>
  );
};

export default Footer;
